...
int icn2053setOEaddrBuffer(ESP32_I2S_DMA_STORAGE_TYPE* buffer, int offset, uint8_t row_cnt, size_t buffer_len, int frame_offset, bool decoder_INT595=false){
	//return 0;
	enum{
		DELAY_INT595_START=2
		};
	enum{
		DELAY_INT595_LEN=ICN2053_ROW_OE_ADD_LEN/2-DELAY_INT595_START
		};
	enum{
		DELAY_INT595_CLK=ICN2053_ROW_OE_ADD_LEN/4-DELAY_INT595_START
		};
...
enum{
	NO_REG=255
	}; //заглушка для отправки данных без команды по LAT
