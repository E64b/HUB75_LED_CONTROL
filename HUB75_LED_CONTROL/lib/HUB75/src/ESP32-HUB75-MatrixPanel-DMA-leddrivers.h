...
//битность регистров
enum{
	DRIVER_BITS=sizeof(driver_reg_t)*8
	};
//enum{DRIVER_BITS = 16};
...
enum{
	ICN2038_PREFIX_CNT=0
	}; //число DMA буферов префиксов
enum{
	ICN2038_SUFFIX_CNT=2
	}; //число DMA буферов суффиксов
enum{
	ICN2038_DSUFFIX_CNT=2
	}; //число наборов DMA дескрипторов на суффикс

//именованные константы для ICN2053
enum{
	ICN2053_PREFIX_CNT=1
	}; //число DMA буферов префиксов
enum{
	ICN2053_SUFFIX_CNT=1
	}; //число DMA буферов суффиксов
enum{
	ICN2053_DSUFFIX_CNT=2
	}; //число наборов DMA дескрипторов на суффикс
...
enum{
	ICN2053_ROW_OE_CNT=138
	}; //число OE импульсов для переключения строки
enum{
	ICN2053_ROW_OE_ADD_LEN=30
	}; //пауза между строчными импульсами OE (для 138 - опциональна, для 595 - нужна)
enum{
	ICN2053_ROW_OE_LEN=ICN2053_ROW_OE_CNT*2+ICN2053_ROW_OE_ADD_LEN
	}; //общая длина в такатах на одну строку OE (*2 - подъем+спад)
